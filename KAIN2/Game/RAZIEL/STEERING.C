...
int ProcessMovement(struct _Instance* instance, long* controlCommand, struct GameTracker* GT)  // Matching - 100%
{
		...
		if (rc == 0)
		{
			if (Raziel.Senses.EngagedMask & 0x40)
			{
				...
				if (((MON_FacingOffset(Raziel.Senses.EngagedList[6].instance, instance) & 0xFFF) - 683) >= 2731U)
				{
					...
					AngleMoveToward(&instance->rotation.z, angle, gameTrackerX.timeMult / 32);
				}
			}
		}
		else
	...
	case 10:
	{
		...
		AngleMoveToward(&instance->rotation.z, angle, gameTrackerX.timeMult / 16);
	...
	case 13:
	{
		...
		AngleMoveToward(&instance->rotation.z, temp, gameTrackerX.timeMult / 16);
